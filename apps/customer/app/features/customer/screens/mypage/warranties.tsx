    ...
    <div className="min-h-screen bg-[#F5F5F0]">
      <div className="mx-auto max-w-2xl px-6 py-10">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">{t("customer:mypage.warranties.title")}</h1>
        </div>
        ...
        <Link to="/customer/warranty" className="block mb-6">
          <div className="bg-[#FF6B35] rounded-2xl p-5 flex items-center justify-between hover:bg-[#FF6B35]/90 transition-colors group">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-white font-semibold text-lg">{t("customer:mypage.warranties.register")}</p>
                <p className="text-white/80 text-sm">{t("customer:mypage.warranties.registerDescription")}</p>
              </div>
            </div>
        ...
        {warranties.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 font-medium">{t("customer:mypage.warranties.noWarranties")}</p>
            <p className="text-sm text-gray-400 mt-1">
              {t("customer:mypage.warranties.noWarrantiesDescription")}
            </p>
          </div>
        ) : (
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-900 text-lg">
                          {warranty.product_name || t("customer:mypage.warranties.product")}
                        </h3>
                      ...
                      <Badge className={`${status.color} px-3 py-1 rounded-full font-medium`}>
                        ...
                        {t(`customer:mypage.warranties.status.${warranty.status}`)}
                      </Badge>
                    </div>
                    
                    <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                      <div className="text-sm text-gray-400 space-y-0.5">
                        <p>{t("customer:mypage.warranties.warrantyNumber")}: {warranty.warranty_number}</p>
                        {warranty.warranty_start && (
                          <p>
                            {t("customer:mypage.warranties.warrantyPeriod")}: {warranty.warranty_start} ~ {warranty.warranty_end}
                          </p>
                        )}
                      </div>
