  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 flex items-center justify-between p-4 text-white">
        <div className="text-lg font-bold">{t("sleep-analysis:result.share.storyCardSave", { defaultValue: "스토리 카드 저장" })}</div>
        <button
          onClick={onClose}
      ...
      >
        <img
          src={imageUrl}
          alt={t("sleep-analysis:result.share.storyCardAlt", { score, defaultValue: `수면 분석 결과 ${score}점` })}
      ...
      <div className="flex-shrink-0 p-6 text-center">
        <div className="bg-white/10 rounded-2xl p-5 max-w-sm mx-auto">
          <p className="text-white text-lg font-bold mb-2">
            {t("sleep-analysis:result.share.longPressToSave", { defaultValue: "이미지를 길게 눌러서 저장하세요" })}
          </p>
          <p className="text-white/70 text-sm">
            {t("sleep-analysis:result.share.shareToInstagram", { defaultValue: "저장 후 인스타그램 스토리에 공유해보세요!" })}
          </p>
        </div>
        ...
        <a
          href={imageUrl}
          download={`${t("sleep-analysis:result.title")}-${score}.png`}
          ...
          onClick={(e) => e.stopPropagation()}
        >
          {t("sleep-analysis:result.share.save")}
        </a>
      </div>
    </div>
  );
