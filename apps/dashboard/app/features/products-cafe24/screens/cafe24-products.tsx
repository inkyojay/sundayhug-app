...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  if (optionFilter === "hasOptions") {
    productsWithVariants = productsWithVariants.filter((p: any) => p.variants && p.variants.length > 1);
  ...
  if (stockFilter === "outOfStock") {
    productsWithVariants = productsWithVariants.filter((p: any) => 
      p.variants?.some((v: Cafe24Variant) => v.stock_quantity <= 0)
    );
  } else if (stockFilter === "lowStock") {
    productsWithVariants = productsWithVariants.filter((p: any) => 
      p.variants?.some((v: Cafe24Variant) => v.stock_quantity > 0 && v.stock_quantity <= v.safety_stock)
    );
  }
  ...
  if (colorFilter !== "all") {
    productsWithVariants = productsWithVariants.filter((p: any) => {
      return p.variants?.some((v: Cafe24Variant) => {
        const mapped = v.sku ? internalProductsMapForFilter.get(v.sku) : null;
  ...
  if (sortBy === "color" || sortBy === "size") {
    productsWithVariants.sort((a: any, b: any) => {
      const getFirstMappedValue = (product: any, field: "color_kr" | "sku_6_size") => {
        const firstVariant = product.variants?.[0];
        const mapped = firstVariant?.sku ? internalProductsMapForFilter.get(firstVariant.sku) : null;
        return mapped?.[field] || "";
      };
  ...
  const allProductsWithVariants = (products || []).map((product: Cafe24Product) => ({
    ...product,
    variants: (variants || []).filter((v: Cafe24Variant) => v.product_no === product.product_no),
  }));
  ...
  allVariants.forEach((v: Cafe24Variant) => {
    const mapped = v.sku ? internalProductsMapForFilter.get(v.sku) : null;
  ...
  const stats = {
    ...
    outOfStock: allVariants.filter((v: Cafe24Variant) => v.stock_quantity <= 0).length,
    lowStock: allVariants.filter((v: Cafe24Variant) => v.stock_quantity > 0 && v.stock_quantity <= v.safety_stock).length,
...
export default function Cafe24Products({ loaderData }: Route.ComponentProps) {
  const { products, stats, internalProducts, availableColors, availableSizes, error, filters } = loaderData;
  ...
  const handleMappingUpdate = () => {
    ...
    mappingFetcher.submit(
      {
        actionType: "mapSku",
        variantId: mappingModal.variantId,
  ...
  const buildUrl = (overrides: Record<string, string | null> = {}) => {
    ...
    return `/dashboard/products-cafe24${queryString ? `?${queryString}` : ""}`;
  };
  ...
  const handleExportCSV = () => {
    const headers = ["제품코드", "제품명", "옵션", "SKU", "판매가", "재고", "안전재고", "상태"];
    const rows: string[][] = [];
    
    products.forEach((product: Cafe24Product) => {
      product.variants?.forEach((variant: Cafe24Variant) => {
        rows.push([
          product.product_code,
          product.product_name,
          formatOptionName(variant.options),
          variant.sku || "",
          String(product.price + variant.additional_price),
          String(variant.stock_quantity),
          String(variant.safety_stock),
          variant.selling === "T" ? "판매중" : "판매중지",
        ]);
      });
    });
    ...
    link.href = url;
    link.download = `cafe24_products_${new Date().toISOString().split("T")[0]}.csv`;
    link.click();
  };
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
        ...
        <CardContent className="pt-4">
          <div className="flex flex-wrap gap-4 items-center">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {hasActiveFilters && (
                <Button variant="outline" size="sm" onClick={() => window.location.href = "/dashboard/products-cafe24"}>
                  초기화
                </Button>
              )}
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            행을 클릭하면 옵션(Variants)을 확인할 수 있습니다. SKU를 내부 제품과 매핑하세요.
          </CardDescription>
        </CardHeader>
            ...
            <Table>
              ...
              <TableBody>
                {products.map((product: Cafe24Product) => {
                  ...
                  const firstVariant = product.variants?.[0];
                  const firstMappedProduct = firstVariant?.sku ? internalProductsMap.get(firstVariant.sku) : null;
                  ...
                  return (
                    <>
                      ...
                      <TableRow 
                        ...
                        onClick={() => !isSingleOption && toggleProduct(product.product_no)}
                      >
                        <TableCell>
                          {!isSingleOption && (
                        ...
                        <TableCell>
                          ...
                          {!firstMappedProduct?.color_kr && <span className="text-muted-foreground text-xs">-</span>}
                        </TableCell>
                        <TableCell>
                          <SizeBadge size={firstMappedProduct?.sku_6_size} />
                          {!firstMappedProduct?.sku_6_size && <span className="text-muted-foreground text-xs">-</span>}
                        </TableCell>
                        ...
                        <TableCell>
                          {outOfStockVariants > 0 ? (
                            <Badge variant="destructive">{outOfStockVariants}개</Badge>
                          ) : (
                              ...
                              <Table>
                                <TableHeader>
                                  <TableRow>
                                    ...
                                    <TableHead className="w-[90px]">추가금액</TableHead>
                                ...
                                <TableBody>
                                  {product.variants.map((variant: Cafe24Variant) => {
                                    const mappedProduct = variant.sku ? internalProductsMap.get(variant.sku) : null;
                                    ...
                                    return (
                                      <TableRow key={variant.variant_code}>
                                        ...
                                        <TableCell>
                                          <Badge 
                                            variant={variant.stock_quantity <= 0 ? "destructive" : variant.stock_quantity <= variant.safety_stock ? "outline" : "secondary"}
                                            className={variant.stock_quantity <= variant.safety_stock && variant.stock_quantity > 0 ? "border-yellow-500 text-yellow-700" : ""}
                                          >
